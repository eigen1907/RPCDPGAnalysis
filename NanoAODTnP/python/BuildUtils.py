...
def rms_and_error(value_sum: np.ndarray, value_sumsq: np.ndarray, counts: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    mask = counts > 0
    mean = np.zeros_like(value_sum, dtype=np.float64)
    mean[mask] = value_sum[mask] / counts[mask]
    variance = np.zeros_like(mean)
    ...
    rms[mask] = np.sqrt(variance[mask])
    yerr = np.zeros_like(mean)
    yerr[mask] = rms[mask] / np.sqrt(2.0 * counts[mask])
    return mask, rms[mask], yerr[mask]
