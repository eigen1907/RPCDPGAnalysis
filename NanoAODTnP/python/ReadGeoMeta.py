...
def roll_mask_series(years: int | str | Sequence[int | str]) -> pd.Series:
    ...
    return pd.Series(True, index=sorted(masked_names), dtype=bool)


@dataclass(frozen=True)
class RollMapSpec:
    name: str
    values_by_roll: pd.Series
    value_label: str
    cmap: str
    vmin: float
    vmax: float
    excluded_by_roll: pd.Series
...
def build_roll_maps(
    ...
    for roll in (RPCRoll.from_row(row) for _, row in geom.iterrows()):
    ...
    for detector_unit, rolls in unit_to_rolls.items():
        ...
        for spec in specs:
            values = spec.values_by_roll.reindex(roll_names).to_numpy(dtype=np.float64)
            inactive = ~np.isfinite(values)
            excluded = spec.excluded_by_roll.reindex(roll_names, fill_value=False).to_numpy(dtype=bool)
            maps.append(
                RollMap(
