...
COUNT_MOMENTUM_EDGES = regular_edges(0.0, 300.0, 60)
VALUE_MOMENTUM_EDGES = np.asarray([
    0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0,
    50.0, 60.0, 70.0, 80.0, 100.0, 125.0, 150.0, 200.0, 250.0, 300.0,
], dtype=np.float64)
PAIR_MUON_PT_EDGES = regular_edges(0.0, 300.0, 300)
...
RPC_RHO_EDGES = regular_edges(0.0, 80.0, 80)
RPC_PV_EDGES = integer_edges(0, 100)
RPC_Q_OVER_P_EDGES = regular_edges(-0.2, 0.2, 80)
RPC_DXDZ_EDGES = regular_edges(-1.5, 1.5, 300)
RPC_ABS_DXDZ_EDGES = regular_edges(0.0, 1.5, 150)
RPC_RESIDUAL_X_EDGES = regular_edges(-100.0, 100.0, 400)
RPC_2D_ETA_EDGES = regular_edges(-3.0, 3.0, 60)
...
RPC_SELECTION_BRANCHES = {
    FIDUCIAL_SELECTION: (
        "probe_pt", "probe_at_rpc_pt", "probe_p", "probe_at_rpc_p",
        "rho", "n_pv", "n_good_pv", "probe_q_over_p",
        "probe_at_rpc_dxdz", ABS_DXDZ_AXIS,
    ),
    MATCHED_SELECTION: tuple(RPC_AXIS_EDGES),
}
CLS_PROFILE_BRANCHES = (
    "probe_pt", "probe_at_rpc_pt", "probe_p", "probe_at_rpc_p",
    "rho", "n_pv", "n_good_pv", "probe_q_over_p", ABS_DXDZ_AXIS,
)
...
KINEMATIC_2D_AXES = {
    "probe_pt_eta": ("probe_pt", VALUE_MOMENTUM_EDGES, "probe_eta", RPC_2D_ETA_EDGES),
    "probe_at_rpc_pt_eta": ("probe_at_rpc_pt", VALUE_MOMENTUM_EDGES, "probe_at_rpc_eta", RPC_2D_ETA_EDGES),
}
...
MOMENTUM_LOSS_STATION_PROFILE_NAMES = tuple(
...
def _histogram_names() -> tuple[str, ...]:
    ...
    names.extend(MOMENTUM_LOSS_STATION_PROFILE_NAMES)
...
def _hist3d(name: str, x_name: str, x_edges: np.ndarray, x_values: np.ndarray, y_name: str, y_edges: np.ndarray, y_values: np.ndarray, z_name: str, z_edges: np.ndarray, z_values: np.ndarray, mask: np.ndarray, weights: np.ndarray | None = None):
    x_values = np.asarray(x_values, dtype=np.float64)
    y_values = np.asarray(y_values, dtype=np.float64)
...
def build_histograms(pair_tree: dict[str, np.ndarray], rpc_tree: dict[str, np.ndarray], roll_blacklist_path: Path) -> dict[str, hist.Hist]:
    ...
    rpc_values = {branch: np.asarray(rpc_tree[branch]) for branch in stored_branches if branch != ABS_DXDZ_AXIS}
    ...
    blacklisted = np.isin(roll_name_values, tuple(load_roll_blacklist(roll_blacklist_path)))
    irpc = np.isin(roll_name_values, tuple(name for name in roll_categories if is_irpc_roll_name(name)))
    accepted = ~blacklisted & ~irpc
    ...
    for plot_name, (x_branch, x_edges, y_branch, y_edges) in KINEMATIC_2D_AXES.items():
        ...
        output[name] = _hist3d(name, x_branch, x_edges, rpc_values[x_branch], y_branch, y_edges, rpc_values[y_branch], "station", station_edges, station_values, matched, cls_values)
