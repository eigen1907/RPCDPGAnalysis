...
def _load_paths(paths: tuple[str, ...]) -> dict[str, DenseHistogram]:
    ...
    optional_names = set(OPTIONAL_HISTOGRAM_NAMES)
    expected_optional_names: set[str] | None = None
    schema_reference: Path | None = None
    for input_path in map(Path, paths):
        with uproot.open(input_path) as root_file:
            ...
            if expected_optional_names is None:
                ...
                schema_reference = input_path
            elif available_optional_names != expected_optional_names:
                missing = sorted(expected_optional_names - available_optional_names)
                extra = sorted(available_optional_names - expected_optional_names)
                raise RuntimeError(
                    "Cannot merge legacy and current histogram schemas: "
                    f"{input_path} differs from {schema_reference}; "
...
def _rebin_fractions(source_edges: np.ndarray, target_edges: np.ndarray) -> np.ndarray:
    ...
    overlap_low = np.maximum(source_edges[:-1, None], target_edges[None, :-1])
    overlap_high = np.minimum(source_edges[1:, None], target_edges[None, 1:])
    overlap = np.clip(overlap_high - overlap_low, 0.0, None)
    return np.divide(overlap, source_widths[:, None], out=np.zeros_like(overlap), where=source_widths[:, None] > 0.0)


def _is_integer_axis(edges: np.ndarray) -> bool:
    edges = np.asarray(edges, dtype=np.float64)
    centers = 0.5 * (edges[:-1] + edges[1:])
    return np.allclose(np.diff(edges), 1.0) and np.allclose(centers, np.rint(centers))


def _rebin_matrix(source_edges: np.ndarray, target_edges: np.ndarray) -> np.ndarray:
    source_edges = np.asarray(source_edges, dtype=np.float64)
    target_edges = np.asarray(target_edges, dtype=np.float64)
    if not _is_integer_axis(source_edges):
        return _rebin_fractions(source_edges, target_edges)
    centers = 0.5 * (source_edges[:-1] + source_edges[1:])
    bins = np.searchsorted(target_edges, centers, side="right") - 1
    matrix = np.zeros((len(source_edges) - 1, len(target_edges) - 1), dtype=np.float64)
    selected = (bins >= 0) & (bins < len(target_edges) - 1)
    matrix[np.nonzero(selected)[0], bins[selected]] = 1.0
    return matrix
...
def _rebin_counts(counts: np.ndarray, source_edges: np.ndarray, target_edges: np.ndarray) -> np.ndarray:
    ...
    if len(source_edges) == len(target_edges) and np.allclose(source_edges, target_edges):
        return counts
    matrix = _rebin_matrix(source_edges, target_edges)
    return np.matmul(counts, matrix).astype(np.float64, copy=False)


def _rebin_counts_axis(values: np.ndarray, source_edges: np.ndarray, target_edges: np.ndarray, axis: int) -> np.ndarray:
    ...
    rebinned = np.matmul(_rebin_matrix(source_edges, target_edges).T, flat)
    reshaped = rebinned.reshape((len(target_edges) - 1, *moved.shape[1:]))
    return np.moveaxis(reshaped, 0, axis)


def _rebin_counts_2d(values: np.ndarray, source_x_edges: np.ndarray, source_y_edges: np.ndarray, target_x_edges: np.ndarray, target_y_edges: np.ndarray) -> np.ndarray:
    rebinned_x = _rebin_counts_axis(values, source_x_edges, target_x_edges, axis=0)
    return _rebin_counts_axis(rebinned_x, source_y_edges, target_y_edges, axis=1)


def _plot_2d_name_and_edges(plot, source_x_edges: np.ndarray, source_y_edges: np.ndarray) -> tuple[str, np.ndarray, np.ndarray]:
    return (
        plot["name"],
        np.asarray(plot.get("x_edges", source_x_edges), dtype=np.float64),
        np.asarray(plot.get("y_edges", source_y_edges), dtype=np.float64),
    )


def _rebin_grouped_2d(histogram: DenseHistogram, group: str, target_x_edges: np.ndarray, target_y_edges: np.ndarray) -> np.ndarray:
    return _rebin_counts_2d(
        _histogram_2d_by_group(histogram, group),
        _edges(histogram, 0),
        _edges(histogram, 1),
        target_x_edges,
        target_y_edges,
...
def _profile_2d_arrays_by_group(histograms: dict[str, DenseHistogram], name: str, group: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def _project_2d(values: np.ndarray, axis: int) -> np.ndarray:
    if axis not in (0, 1):
        raise ValueError(f"unsupported 2D projection axis: {axis}")
    return np.sum(values, axis=1 - axis)


def _count_projection_by_group(histograms: dict[str, DenseHistogram], selection: str, name: str, group: str, axis: int) -> tuple[np.ndarray, np.ndarray]:
    histogram = histograms[count_2d_station_name(selection, name)]
    return _project_2d(_histogram_2d_by_group(histogram, group), axis), _edges(histogram, axis)


def _profile_projection_by_group(histograms: dict[str, DenseHistogram], name: str, group: str, axis: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    sums, sumsqs, counts, x_edges, y_edges = _profile_2d_arrays_by_group(histograms, name, group)
    return (
        _project_2d(sums, axis),
        _project_2d(sumsqs, axis),
        _project_2d(counts, axis),
        (x_edges, y_edges)[axis],
    )
...
def load_efficiency_2d_results(histograms: dict[str, DenseHistogram], plots: Sequence, group: str = "all") -> dict[str, Efficiency2DResult]:
    results = {}
    for plot in plots:
        source_name = plot["name"]
        total_histogram = histograms[count_2d_station_name(FIDUCIAL_SELECTION, source_name)]
        passed_histogram = histograms[count_2d_station_name(MATCHED_SELECTION, source_name)]
        name, target_x_edges, target_y_edges = _plot_2d_name_and_edges(plot, _edges(total_histogram, 0), _edges(total_histogram, 1))
        results[name] = Efficiency2DResult(
            _rebin_grouped_2d(total_histogram, group, target_x_edges, target_y_edges),
            _rebin_grouped_2d(passed_histogram, group, target_x_edges, target_y_edges),
            target_x_edges,
            target_y_edges,
        )
    return results
...
def load_cls_2d_results(histograms: dict[str, DenseHistogram], plots: Sequence, group: str = "all") -> dict[str, Profile2DResult]:
    results = {}
    for plot in plots:
        source_name = plot["name"]
        sums, sumsqs, counts, source_x_edges, source_y_edges = _profile_2d_arrays_by_group(histograms, source_name, group)
        name, target_x_edges, target_y_edges = _plot_2d_name_and_edges(plot, source_x_edges, source_y_edges)
        results[name] = Profile2DResult(
            _rebin_counts_2d(sums, source_x_edges, source_y_edges, target_x_edges, target_y_edges),
            _rebin_counts_2d(sumsqs, source_x_edges, source_y_edges, target_x_edges, target_y_edges),
            _rebin_counts_2d(counts, source_x_edges, source_y_edges, target_x_edges, target_y_edges),
            target_x_edges,
            target_y_edges,
        )
    return results
...
def _category_profile(labels: np.ndarray, sums: np.ndarray, sumsqs: np.ndarray, counts: np.ndarray, all_labels: np.ndarray | None = None) -> CategoryProfileResult:
    ...
    return CategoryProfileResult(
        unique,
        np.bincount(inverse, weights=sums, minlength=len(unique)).astype(np.float64),
        np.bincount(inverse, weights=sumsqs, minlength=len(unique)).astype(np.float64),
        np.bincount(inverse, weights=counts, minlength=len(unique)).astype(np.int64),
    )
...
def _category_efficiency(labels: np.ndarray, total: np.ndarray, passed: np.ndarray, all_labels: np.ndarray | None = None) -> CategoryEfficiencyResult:
    ...
    return CategoryEfficiencyResult(
        unique,
        np.bincount(inverse, weights=total, minlength=len(unique)).astype(np.int64),
        np.bincount(inverse, weights=passed, minlength=len(unique)).astype(np.int64),
    )


def _run_profile_arrays(histograms: dict[str, DenseHistogram], key: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def _run_efficiency_arrays(histograms: dict[str, DenseHistogram], key: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    indices = STATION_INDICES[key]
...
def _run_profiles(histograms: dict[str, DenseHistogram], keys: Sequence[str]):
    by_run = {}
    for key in keys:
        indices = STATION_INDICES[key]
        profile = histograms[CLS_RUN_STATION_PROFILE]
        counts = histograms[count_run_station_name(MATCHED_SELECTION)]
        by_run[key] = CategoryProfileResult(
            RUNS.copy(),
            np.sum(_contents(profile)[:, indices], axis=1),
            np.sum(_contents(profile, variance=True)[:, indices], axis=1),
            np.sum(_contents(counts)[:, indices], axis=1),
...
def _run_efficiencies(histograms: dict[str, DenseHistogram], keys: Sequence[str]):
    by_run = {}
    for key in keys:
        indices = STATION_INDICES[key]
        by_run[key] = CategoryEfficiencyResult(
            RUNS.copy(),
            np.sum(_contents(histograms[count_run_station_name(FIDUCIAL_SELECTION)])[:, indices], axis=1),
            np.sum(_contents(histograms[count_run_station_name(MATCHED_SELECTION)])[:, indices], axis=1),
        )
    return by_run


def _elapsed_profiles(histograms: dict[str, DenseHistogram], run_meta, keys: Sequence[str]):
    all_time_bin_timestamps = np.unique(np.asarray(run_meta.time_bin_timestamps, dtype="datetime64[s]"))
    by_elapsed = {}
    for key in keys:
        runs, sums, sumsqs, counts = _run_profile_arrays(histograms, key)
        by_elapsed[key] = _category_profile(run_meta.lookup_time_bin_timestamps(runs), sums, sumsqs, counts, all_time_bin_timestamps)
    return by_elapsed


def _elapsed_efficiencies(histograms: dict[str, DenseHistogram], run_meta, keys: Sequence[str]):
    all_time_bin_timestamps = np.unique(np.asarray(run_meta.time_bin_timestamps, dtype="datetime64[s]"))
    by_elapsed = {}
    for key in keys:
        runs, total, passed = _run_efficiency_arrays(histograms, key)
        by_elapsed[key] = _category_efficiency(run_meta.lookup_time_bin_timestamps(runs), total, passed, all_time_bin_timestamps)
    return by_elapsed
...
def load_pair_results(histograms: dict[str, DenseHistogram], plots):
    ...
    probe_eta_phi = histograms[pair_eta_phi_name("probe")]
    tag_eta_phi = histograms[pair_eta_phi_name("tag")]
    probe_2d = _contents(probe_histogram)
    tag_2d = _contents(tag_histogram)
    probe_eta_phi_2d = _contents(probe_eta_phi)
    tag_eta_phi_2d = _contents(tag_eta_phi)
    source_1d = {
        "pair_mass": (_contents(pair_mass), _edges(pair_mass)),
        "probe_pt": (np.sum(probe_2d, axis=0), _edges(probe_histogram, 1)),
        "probe_eta": (np.sum(probe_2d, axis=1), _edges(probe_histogram, 0)),
        "probe_phi": (np.sum(probe_eta_phi_2d, axis=0), _edges(probe_eta_phi, 1)),
        "tag_pt": (np.sum(tag_2d, axis=0), _edges(tag_histogram, 1)),
        "tag_eta": (np.sum(tag_2d, axis=1), _edges(tag_histogram, 0)),
        "tag_phi": (np.sum(tag_eta_phi_2d, axis=0), _edges(tag_eta_phi, 1)),
    }
    if PAIR_Q_OVER_P_HISTOGRAM in histograms:
        q_over_p = histograms[PAIR_Q_OVER_P_HISTOGRAM]
        source_1d["probe_q_over_p"] = (_contents(q_over_p), _edges(q_over_p))
    return {
        plot["name"]: _count1d(*source_1d[plot["branch"]], plot["edges"])
        for plot in plots
        if plot["branch"] in source_1d
    }


def load_pair_2d_results(histograms: dict[str, DenseHistogram], plots):
    source_2d = {
        "probe_pt_eta": histograms[pair_kinematics_name("probe")],
        "probe_eta_phi": histograms[pair_eta_phi_name("probe")],
        "tag_pt_eta": histograms[pair_kinematics_name("tag")],
        "tag_eta_phi": histograms[pair_eta_phi_name("tag")],
    }
    results = {}
    for plot in plots:
        source = source_2d[plot["branch"]]
        source_x_edges = _edges(source, 0)
        source_y_edges = _edges(source, 1)
        target_x_edges = np.asarray(plot.get("x_edges", source_x_edges), dtype=np.float64)
        target_y_edges = np.asarray(plot.get("y_edges", source_y_edges), dtype=np.float64)
        results[plot["name"]] = Hist2DResult(
            _rebin_counts_2d(_contents(source), source_x_edges, source_y_edges, target_x_edges, target_y_edges),
            target_x_edges,
            target_y_edges,
        )
    return results
...
def load_rpc_results(histograms: dict[str, DenseHistogram], count_plots, mean_plots, run_meta, trend_keys: Sequence[str]):
    ...
    for plot in mean_plots:
        if "source_2d" in plot:
            if (
                cls_profile_2d_station_name(plot["source_2d"]) not in histograms
                or count_2d_station_name(MATCHED_SELECTION, plot["source_2d"]) not in histograms
            ):
                continue
            sums, sumsqs, counts, source_edges = _profile_projection_by_group(histograms, plot["source_2d"], plot["region"], plot["axis"])
        else:
            if cls_profile_station_name(plot["x_branch"]) not in histograms:
                continue
        ...
        mean_results[plot["name"]] = Profile1DResult(
            _rebin_counts(sums, source_edges, plot["edges"]),
            _rebin_counts(sumsqs, source_edges, plot["edges"]),
            _rebin_counts(counts, source_edges, plot["edges"]),
            np.asarray(plot["edges"], dtype=np.float64),
        )
    by_elapsed = _elapsed_profiles(histograms, run_meta, trend_keys)
    return count_results, mean_results, _roll_mean_result(histograms), by_elapsed, _run_profiles(histograms, trend_keys)


def load_rpc_rms_results(histograms: dict[str, DenseHistogram], rms_plots):
    results = {}
    for plot in rms_plots:
        profile_name = profile_1d_station_name(plot["value_branch"], plot["x_branch"])
        count_name = count_station_name(MATCHED_SELECTION, plot["x_branch"])
        if profile_name not in histograms or count_name not in histograms:
            continue
        results[plot["name"]] = _profile1d_by_group(
            histograms,
            plot["value_branch"],
            plot["x_branch"],
            plot["edges"],
...
def load_efficiency_results(histograms: dict[str, DenseHistogram], plots_1d, run_meta, trend_keys: Sequence[str]):
    results_1d = {}
    for plot in plots_1d:
        if "source_2d" in plot:
            if (
                count_2d_station_name(FIDUCIAL_SELECTION, plot["source_2d"]) not in histograms
                or count_2d_station_name(MATCHED_SELECTION, plot["source_2d"]) not in histograms
            ):
                continue
            total, total_edges = _count_projection_by_group(histograms, FIDUCIAL_SELECTION, plot["source_2d"], plot["region"], plot["axis"])
            passed, passed_edges = _count_projection_by_group(histograms, MATCHED_SELECTION, plot["source_2d"], plot["region"], plot["axis"])
        else:
            total = _count_by_group(histograms, FIDUCIAL_SELECTION, plot["branch"], plot["region"])
            total_edges = _edges(histograms[count_station_name(FIDUCIAL_SELECTION, plot["branch"])])
            passed = _count_by_group(histograms, MATCHED_SELECTION, plot["branch"], plot["region"])
            passed_edges = _edges(histograms[count_station_name(MATCHED_SELECTION, plot["branch"])])
        results_1d[plot["name"]] = Efficiency1DResult(
            _rebin_counts(total, total_edges, plot["edges"]),
            _rebin_counts(passed, passed_edges, plot["edges"]),
            np.asarray(plot["edges"], dtype=np.float64),
        )
    by_elapsed = _elapsed_efficiencies(histograms, run_meta, trend_keys)
    return results_1d, _roll_efficiency_result(histograms), by_elapsed, _run_efficiencies(histograms, trend_keys)
