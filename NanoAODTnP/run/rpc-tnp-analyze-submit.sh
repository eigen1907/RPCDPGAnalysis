...
Options:
  ...
  --output-base PATH      Histogram output base directory
                          (default: /eos/user/j/joshin/rpc/tnp-hist,
                           or /eos/user/j/joshin/rpc/tnp-hist-wo-blacklist with --no-blacklist)
  --roll-blacklist PATH   Roll blacklist file to apply (default: data/blacklist/roll/blackListYYYY.txt)
  --no-blacklist          Use an empty roll blacklist file
...
if [[ "${NO_BLACKLIST}" -eq 1 && -n "${ROLL_BLACKLIST_PATH}" ]]; then
    echo "[error] --no-blacklist and --roll-blacklist cannot be used together" >&2
    exit 2
fi
if [[ "${NO_BLACKLIST}" -eq 1 && "${output_base_set}" -eq 0 ]]; then
    OUTPUT_BASE="/eos/user/j/joshin/rpc/tnp-hist-wo-blacklist"
fi
...
if [[ "${NO_BLACKLIST}" -eq 1 ]]; then
    ITEMS_ALL_BASE="${LOG_BASE}/items/no-blacklist/all"
    ITEMS_RESUBMIT_BASE="${LOG_BASE}/items/no-blacklist/resubmit"
else
...
if [[ "${NO_BLACKLIST}" -eq 1 ]]; then
    roll_blacklist_file="${ROLL_BLACKLIST_DIR}/noBlacklist.txt"
