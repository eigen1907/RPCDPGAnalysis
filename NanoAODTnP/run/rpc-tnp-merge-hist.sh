...
Options:
  ...
  --no-blacklist      Use no-blacklist input/output defaults
...
if [[ "${NO_BLACKLIST}" -eq 1 && "${input_base_set}" -eq 0 ]]; then
    INPUT_BASE="/eos/user/j/joshin/rpc/tnp-hist-wo-blacklist"
fi
if [[ "${NO_BLACKLIST}" -eq 1 && "${output_base_set}" -eq 0 ]]; then
    OUTPUT_BASE="/eos/user/j/joshin/rpc/tnp-hist-wo-blacklist-merged"
fi
